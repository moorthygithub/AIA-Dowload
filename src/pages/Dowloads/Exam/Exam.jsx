...
function Exam() {
  ...
  const [downloadDelivery, setExamDownload] = useState({
    exam_date_from: firstdate,
    exam_date_to: todayback,
    student_uid: "",
  });
  ...
  const onSubmit = (e) => {
    ...
    if (v) {
      ...
      axios({
        url: BASE_URL + "/panel-download-exam",
        method: "POST",
        data,
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const onReportView = (e) => {
    e.preventDefault();
    localStorage.setItem("exam_date_from", downloadDelivery.exam_date_from);
    localStorage.setItem("exam_date_to", downloadDelivery.exam_date_to);
